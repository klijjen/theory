...
ООП облегчает работу с большими проектами, разбивая программу на более мелкие независимые сущности, которые легче понимать: объекты содержат в себе состояние, инкапсулируют логику.

## Основные принципы ООП

**Инкапсуляция** – скрытие внутренней реализации объекта и предоставление доступа только через публичные методы.

**Наследование** – возможность создания нового класса на основе существующего с переиспользованием его свойств и методов.

**Полиморфизм** – возможность объектов с одинаковой спецификацией иметь разную реализацию.

  - Перегрузка методов (overloading) – несколько методов с одним именем, но разными параметрами.

  - Переопределение методов (overriding) – замена реализации метода родителя в классе-потомке (@Override).

**Абстракция** – выделение главных характеристик объекта, игнорируя несущественные детали.

### Инкапсуляция 

Инкапсуляция - это механизм сокрытия реализации 
...
Наследование — это механизм создания новой сущности на базе уже существующей
...
Полиморфизм - это механизм, который позволяет единообразно обращаться к объектам различных классов (обычно имеющих общего предка)

Пример:

У макбука резко появились супер ядра, значение которых нужно возвращать вместо количества ядер обычных. Мы можем либо написать свой метод (getSuperDuperCores), либо убрать наследование от класса компьютер.
...
Получим 2, а не 16. Потому что метод getCoresHalfed использует класс Computer, и даже если мы очень захотим, в этом методе не получится использовать метод getSuperDuperCores, так как мы работаем с объектом типа Computer, у которого он просто не определён.
...
Абстракция - это выделение общих характеристик объекта, его свойств и методов, при игнорировании деталей реализации. Это позволяет создавать более простые, управляемые и гибкие системы
...
Когда мы создаёт объекты: мы используем ключевое слово `new`, которое вызывает **конструктор** - специальный статический метод, который создаёт новый экземпляр этого класса.
...
Если у нас есть класс A (не виртуальный, а вполне реально используемый в  коде) и отнаследованный от него класс B, то если мы заменим все использования класса A на B, ничего не должно измениться в работе  программы (https://habr.com/ru/articles/83269, очень советую почитать, если не поняли).
...
Его формулировка - зависимости должны быть основаны на абстракциях, а не на конкретных реализациях (https://habr.com/ru/articles/313796/)
